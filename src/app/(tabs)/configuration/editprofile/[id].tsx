    ...
    useEffect(() => {
        ...
        async function getProfile() {
            try {
                ...
                setName(userApi.data[0].user.name);
                setEmail(userApi.data[0].user.email);

                setRole(userApi.data[0].user.permission);
            } catch (error) {
                console.log(error);
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (user?.permission === "admin" && id !== user.id) {
                const updatePorfileForAdmin = await updateProfile(
                    data,
                    String(id)
                );
            ...
            if (updateUser.message === "Usuário atualizado com sucesso!") {
        ...
        <Container>
            <InputContainer>
                ...
                <StyledTextInput
                    ...
                    style={{
                        backgroundColor:
                            user?.id === id && user.permission !== "admin"
                                ? "#ccc"
                                : "",
                    }}
                />
